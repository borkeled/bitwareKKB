...
{
    var->style = gui_style(); // Reset to unscaled defaults to prevent cumulative double-scaling on DPI changes
    ...
    style->scrollbar_size = s_(5.f);
    style->scrollbar_content_padding = s_(6.f);
    style->scrollbar_border_padding = s_(0, 0);
    
    // Scale standard layout variables to keep sizing and rounding consistent
    style->child_rounding = s_(style->child_rounding);
    style->frame_padding = s_(style->frame_padding);
    style->frame_rounding = s_(style->frame_rounding);
    style->popup_rounding = s_(style->popup_rounding);
    style->grab_min_size = s_(style->grab_min_size);
    style->grab_rounding = s_(style->grab_rounding);
    style->tab_rounding = s_(style->tab_rounding);
    style->scrollbar_rounding = s_(style->scrollbar_rounding);
}
